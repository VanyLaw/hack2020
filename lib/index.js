...
//   return readmes;
// };
const getFilesFromGit = async (url) => {
    const result = await httpClient.get(url);
    return result.data;
};
const getReadmeFiles = async () => {
    //oauth
    await httpClient.get("?access_token=646870601e9a9e7c432c38c076c33e27f9f46bad");
    ...
    for (const file of result) {
        if (typeof file.name === "string" && file.name.endsWith(".md")) {
            const response = await axios_1.default.get(file.download_url);
...
const getLinks = async () => {
    ...
    // get all the links
    for (const file of files) {
        const result = file.content.match(/(((http|https|ftp):\/\/[\w\-_]+(\.[\w\-_]+)+)|~\/|\.+\/)([\w\-\.,@?^=%&:/~\+#]*[\w\-\@?^=%&/~\+#])+/g);
...
const getSampleCommits = async (date) => {
    const links = await getCodeLinks();
    const mdSampleCommitMap = {};
    for (const key in links) {
...
const getMDFileCommits = async (date) => {
    const mdCommitMap = {};
